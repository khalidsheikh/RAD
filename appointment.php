<?php
	...
	else
	{
		...
		if(isset($_POST['title']) && isset($_POST['timeStart']) && isset($_POST['timeStop']) && isset($_POST['date']) && isset($_POST['notes']) && isset($_POST['location']) && isset($_POST['bookable']) && isset($_POST['parentAppointment']))
		{
			$title = $_POST['title'];
			...
			$date = $_POST['date'];
			$notes = $_POST['notes'];
			$parentAppointment = $_POST['parentAppointment'];
			...
			$data = array('location'=>$location,'participants'=>$participants,'parentAppointment'=>$parentAppointment, 'title'=>$title, 'startTime'=>$timeStart, 'endTime'=>$timeStop, 'date'=>$date, 'notes'=>$notes, 'recurrence'=>$recurrence, 'bookable'=>$bookable);	
			$json = json_encode($data);
			$url = "http://cortexapi.ddns.net:8080/api/addNewAppointment";
		...
		$appArray = array();
		$url = "http://cortexapi.ddns.net:8080/api/getAllLecturer";
		$options = array(
		...
		<div class='container'>
			<div class='row'>
				<div class='col-md-12' style='position:relative'>
					...
					<form role='form' id='slotForm' method='post' action='appointment.php' >
						...
						<div class="form-group">
							...
							<select class='form-control' id='lecturer' name='lecturer'>
								...
								<?php foreach($appArray as $app) { ?>
									<option data-username="<?php echo $app[3]; ?>" value="<?php echo $app[2]; ?>"><?php echo $app[1]; ?></option>
								<?php } ?>
							</select>
							<p id='lecturerAlert' class='text text-danger hidden'></p>
						</div>
						<div class="form-group">
							<label for="parentAppointment">Parent Appointment *</label>
							<select class='form-control' id='parentAppointment' name='parentAppointment'>
								
							</select>
							<p id='parentAppointmentAlert' class='text text-danger hidden'></p>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<p id='locationAlert' class='text text-danger hidden'></p>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<p id='participantsAlert' class='text text-danger hidden'></p>
						</div>
						<div class="form-group">
							<label for="recurringWeekly">Recurrence *</label>
							<select class='form-control' id='recurringWeekly' name='recurrence'>
						...
						<button id='submit' type="submit" class="btn btn-primary btn-lg"><i class=""></i>Create Appointment</button>
					</form>
				</div>
			</div>
...
<script type='text/javascript'>
	$(document).on('ready',function(){
		$('#lecturer').on('change', function(){
			var value = $('#lecturer').val();
			//var data = {'email':value};
			var url = 'http://cortexapi.ddns.net:8080/api/getAvailabilityByPerson/';
			var _url = url + value;
			$('#parentAppointment').html('');
			$.getJSON(_url, function(res){
				if(res.status == 'success') {
					console.log(res.data);
					$.each(res.data, function(){
						var app = this;
						var $option = $('<option/>').attr('value',app.id).text(app.name);
						$('#parentAppointment').append($option);
					});
				} else {
	...
	function validateForm() {
		...
		if(!$('#parentAppointment').val()) {
			$('#parentAppointmentAlert').removeClass('hidden').text('* Parent Appointment is required');
			$('#parentAppointment').addClass('error');
			flag = false;
		}
		if(!$('#slotDate').val()) {
		...
		if(!$('#notes').val()) {
			$('#notesAlert').removeClass('hidden').text('* Notes is required');
			$('#notes').addClass('error');
			flag = false;
		}
		if(!$('#location').val()) {
			$('#locationAlert').removeClass('hidden').text('* Location is required');
			$('#location').addClass('error');
			flag = false;
		}
		if(!$('#participants').val()) {
			$('#participantsAlert').removeClass('hidden').text('* Participants is required');
